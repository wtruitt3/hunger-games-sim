...
class Participant(object):
    ...
    def clean_tile_at_position(self, pos):
        ...
        healStatements=[f'{self.name} found NeoSporin in this arena', f'{self.name}, youfound a Band-aid in the bushes', f'{self.name} has an immune system on 100', f'{self.name} has recovered from injuries', f'{self.name} is so down bad a doctor came to help']
    ...
    def update_position_and_clean(self):
        ...
        if self.room.is_position_in_room(nextPos):
            ...
            self.clean_tile_at_position(self.position)
        else:
            self.direction=random.uniform(0.0,360.0)    
...
room=Room(20,30,2,3,2)
p1=Participant(room,"test2",60,2,2,4,1)
p2=Participant(room,"test1",60,2,2,3,2)
p3=Participant(room,"test3",60,2,2,1,0)
# #print(p1.get_creativity())
#checkFlee([p1,p3,p2])
def Encounter(pList):
    """
    pList - List of participant objects
   ...
   #      timeStep+=1
    ...
    while len(participantList)>1:
        ...
        for participant in participantList:
            participant.update_position_and_clean()
